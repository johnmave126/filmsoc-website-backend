...
def query_user(itsc, attrs = ['displayName']):
    ...
    itsc -- The ITSC account of the person queried
    attrs -- The information to query (default ['displayName'])
...
def update_mailing_list(new_list):
    ...
    new_list -- The list to replace the present
    """
...
def upload_file(filename, file_handler):
    ...
    filename -- The name to be stored in FTP
    file_handler -- The file to be uploaded
    """
...
def send_email(receiver, bcc, subject, body):
    ...
    receiver -- A list of receivers
    bcc -- A list of receivers who will not be shown in to:
